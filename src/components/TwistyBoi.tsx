...
export function TwistyBoi() {
  ...
  return (
    <main className="flex h-4/6 w-screen flex-col items-center justify-between gap-24 py-12">
      <h1 className="rounded-full bg-slate-200 px-8 py-4 text-3xl font-medium">
        Compass Refresh
      </h1>
      <h2 className="-mt-20 text-xl">Coming Soon!</h2>
      <div className="px-y -mt-20 bg-slate-200 px-2 text-xl">
        <select
          value={color}
        ...
        <div id="dna" style={{ animation: "r-y 3s linear infinite" }}>
          {new Array(120).fill(0).map((_, i) => (
            <div
              ...
              className={`border-1 h-1 w-24 animate-pulse transition-all ${getBg(i, color)}`}
            />
          ))}
        </div>
      </div>
